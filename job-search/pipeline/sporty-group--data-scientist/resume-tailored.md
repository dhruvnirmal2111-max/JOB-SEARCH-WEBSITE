...
Data scientist who builds ML that moves a real business number. My clearest proof: a one-vs-all classifier (scikit-learn, TF-IDF to embeddings, GridSearchCV) that cut a client's **A$12B** spend categorisation from a month to a single day's run, plus Prophet forecasting and pricing analysis that surfaced roughly **A$2M** in savings. I work in Python and SQL on a Master of Data Science foundation, and I use AI agents daily, having built multi-agent QA and reporting systems now used company-wide, not just occasional AI-assisted coding.
...
- Approach: led the firm's first ML classifier, a one-vs-all model that keeps high-value spend on manual review and models the long tail automatically. scikit-learn, text features (TF-IDF moving to embeddings) plus engineered features, LogisticRegression with class-imbalance handling, tuned with GridSearchCV and cross-validation.
