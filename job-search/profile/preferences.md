# Job Search Preferences

> Pre-filled by `/intake` from the resume. Items tagged **[ASSUMED]** are my best guess — please correct them.

## Target roles
- Data Engineer
- Analytics Engineer
- Data Scientist
- ML / AI Engineer (given agentic-AI + LLM workflow experience)
- Data Analyst
- Product Analyst
- Procurement Data Analyst

## Seniority
- Mid-level (2+ yrs), targeting **mid → senior**. **[ASSUMED]**

## Locations
- Melbourne, Australia (primary)
- Remote / hybrid anywhere in Australia. **[ASSUMED]**
- Onsite: open within Melbourne. **[ASSUMED]**

## Compensation
- Target / expected base: **AUD 95,000+** (target above 95k; happy higher).
- Minimum acceptable: **AUD 95,000** — filter out roles advertising below this.

## Must-haves
- I have full working rights.

## Dealbreakers
- no AI/ML scope, etc.

## Industries / domains of interest
- Data analytics & consulting, fintech, AI/ML products, government/public sector, supply chain / procurement (existing domain strength).


## Strengths to lead with (from resume)
- 20+ production ETL pipelines; ~15% client cost savings via anomaly detection.
- Agentic AI / multi-agent LLM workflows with Claude (rare, marketable edge).
- Full stack of cloud + IaC: Azure, AWS, Databricks, Terraform, Snowflake, Docker.
- Strong BI: 16+ Tableau dashboards, Power BI adopted by Dept of Transport.

## Notes
- Current role: Data Engineer at PI Data Analytics (Comprara Group), Melbourne, since June 2024.
- Campaign start date: **2026-06-21**
- 30-day target end date: **2026-07-21**
- Goal: signed offer within 30 days.
