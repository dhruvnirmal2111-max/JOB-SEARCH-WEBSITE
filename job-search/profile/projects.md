...
### P3. Product catalog + pricing-decision & supplier dashboards (restaurant chain, AU & UK) `#bi #dashboards #pricing #procurement #catalog #statistics #python #stakeholders`
...
- **What I did:** Multi-part project for a restaurant chain operating across Australia & the UK, which I **owned and ran end-to-end as the sole analyst on the account**. (1) **Built the product catalog** — structured ~3,000–3,500 products into a categorised catalog as the foundation. (2) **Pricing-decision dashboard** — tracks how prices move across products/categories so the client can negotiate better deals on their category mix: an overview level showing potential savings vs. money lost on bad deals, with drill-down to the detail level to pinpoint which products, venues and suppliers are driving the overspend. (3) **Supplier & item-onboarding dashboard** — shows how many suppliers were onboarded and how many new items were added for purchase each month, and flags where a better/cheaper substitute product is available from a supplier, guiding smarter ordering decisions. Ran the client relationship directly — bi-weekly/monthly progress meetings with senior stakeholders (CFO, Chief Procurement Officer, category managers, general managers, and the client-side procurement team).
- **Delivery/impact:** Enabled the client to identify ~**30% savings in one of their largest categories — approximately A$2M in savings over a year** — by surfacing price movement and cost leakage down to the product/venue/supplier level for better-negotiated supplier deals.
...
### P16. MyFacit — analytics & decision-support SaaS for a small hospitality business `#personal #saas #ai #analytics #forecasting #bi`
