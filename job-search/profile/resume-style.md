...
- **Default to 1 page. Always, for every role, unless the owner explicitly asks for 2 pages for a specific application.** The owner has under ~5 years of experience and has repeatedly said the 2-page versions are too big. Do NOT decide on your own that a role is "DS-depth" or "consulting-depth" and therefore deserves 2 pages, that judgment call is the owner's, not yours. A DS or analyst target is still 1 page by default.
- The known exception so far: **7-Eleven** (the owner explicitly asked for 2 pages with engagement-grouped Problem/Approach/Result stories after an interview). Treat 2 pages as opt-in per application, never the default.
- To hit 1 page: use tight single-line achievement bullets (not Problem/Approach/Result triplets, which are a 2-page-only device), keep the 3 to 4 strongest engagements, one-line or drop the Projects section, trim education to degree + institution + dates (no coursework list), and cut the oldest/least-relevant role. Build with `--max-pages 1`. ~9.5 to 10.5pt is fine; do not cram below ~9pt, trim content instead.
