...
- **For product / product-analytics roles, LEAD with MyFacit.** It is the owner's one genuine product credential: a live product he built and owns, defines the metrics for, decides what it tracks, and makes decisions on. On product-analytics JDs (drive product decisions, work with Product/Engineering, define data-tracking, user/engagement/retention metrics, optimise user experience), surface MyFacit prominently, a mention in the summary AND a full multi-bullet entry framed as *owning the metrics, data-tracking and decisions for a live product*, not a one-line project at the bottom. Owner feedback (Brighty, Sep 2026): "they were talking about product, we should have highlighted MyFacit." Keep strictly to the confirmed facts in `projects.md` P16.
...
## Resume length (1 vs 2 pages) — a judgment call, not a fixed rule
Length depends on the situation. It is **not** "always 1 page" and **not** "always 2". Read the signal:

- **Default to 1 page for a first-pass / cold application** — a role coming off the shortlist or a paste-in JD, especially analyst/BAU/early-career targets. The owner has under ~5 years of experience and has said the fuller versions look "too big" for these. Puffy, Keypath, micro1 are examples: 1 page each. Do NOT unilaterally decide a first-pass role is "DS-depth" and blow it out to 2 pages, that overshoot is exactly what the owner pushed back on.
- **Go to 2 pages when depth is genuinely the selling point, and there's a real reason for it** — the strongest reason being **feedback from a recruiter or interviewer** asking for more (the **7-Eleven** case: after the interview the interviewer wanted the connected, engagement-grouped stories, so we split the experience by client/industry into **Problem → what I did → how → Result** sub-bullets, named the actual tools/libraries, and it went to 2 pages). The owner asking directly also triggers 2 pages. When you're deeper in a process for a specific role, richer is often better than shorter.
- **When it's a substantive role and you're genuinely unsure**, offer both (build a 1-page and a 2-page variant in the folder) or ask the owner, rather than guessing. It's fine to keep both variants.
- **To hit 1 page:** tight single-line achievement bullets (not Problem/Approach/Result triplets, which are the 2-page device), keep the 3 to 4 strongest engagements, one-line or drop Projects, trim education to degree + institution + dates, cut the oldest/least-relevant role. Build with `--max-pages 1`. ~9.5 to 10.5pt is fine; don't cram below ~9pt, trim content instead.
- **For the 2-page depth format:** group experience by client/industry, use the Problem → what I did → how → Result sub-bullet structure, name the real tools/libraries (scikit-learn, numpy, pandas, Prophet), label internal vs client work, and show model evolution where true (e.g. TF-IDF → embeddings). Build with `--max-pages 2`.
...
- **Never invent project specifics.** Use only facts in `projects.md` or explicitly confirmed by the owner. For MyFacit, the confirmed facts are now documented in `projects.md` P16 and ARE usable: multi-tenant profitability SaaS; unifies POS, wages and 14 suppliers' invoices; weekly "pulse" (revenue, wage cost %, food cost %, gross margin, one recommended action); revenue forecasting, menu engineering, supplier price-creep detection; design partner Neighbours Cafe (Dom), St Kilda. Do NOT go beyond P16 (e.g. don't invent a tech stack, user counts, or A/B/cohort features it doesn't have).
