...
2. **Research with `WebSearch` / `WebFetch`.** Default focus (per `goals.md`): **remote-first data & AI/tech companies that hire in India, Dubai, or Singapore** — the geographies that survive the move out of Australia. Include strong AU-remote employers too, but lead with roles the owner could keep after relocating. Find companies matching the owner's target roles (Data Analyst / Data Scientist / AI-focused), domains, and **experience level** that they'd genuinely take a role at, even if nothing's posted now. For each capture: why it's a fit, its remote posture + hiring geos, and whether it currently has a relevant open role (`yes/no`).
   - **Match the owner's experience level (~2 years, mid / early-career).** When you check "open role now?", verify the open role is actually **at his level** — mid-level Data Scientist / Data Analyst / Associate / AI Engineer roles asking roughly **1–4 years**. A **Senior / Staff / Lead / Principal / Manager / Head** req he can't realistically land, or an **intern / new-grad** role beneath him, does NOT count as a matching open role. If the only openings are out-of-level, mark open-role `no (only senior)` or `no (only grad)` and keep the company as a relationship target, not an apply target. Note the level of any qualifying role.
   - Aim for 8–12 candidates.

3. **Rank** by fit (role/domain match, **remote posture + India/Dubai/Singapore reach**, **a live role at his experience level**, growth, how much the owner would want to work there). Roles that survive the move to India and sit at his level rank highest.

4. **Append to `job-search/network/target-accounts.md`** — fill Company, Why it's a fit (note remote posture + hiring geos + **level of any open role**), Warmth (`cold` to start), Open role now? (with level, e.g. `yes — mid-level DS` / `no (only senior)`), and Added (today via `date +%F`). Leave `Pursue?` **blank** — these are candidates, not confirmed. Where you can, include the **live role link / source URL** so rows are traceable.
